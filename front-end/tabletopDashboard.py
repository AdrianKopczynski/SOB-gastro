...
class TabletopDashboard(tk.Frame):
    ...
    def create_widgets(self):
        ...
        tk.Button(buttons_frame, text="Back", font=("Arial", 16), bg="gray", fg="white",
                    width=15, height=2, command=lambda: self.manager.switch_to("TabletopEditor")).grid(row=0, column=1, pady=10)
            
        tk.Button(buttons_frame, text="New Order", font=("Arial", 16), bg="gray", fg="white",
                      width=15, height=2, command=self.create_order).grid(row=1, column=1, pady=10)
        
        self.button_edit = tk.Button(buttons_frame, text="Edit Order", font=("Arial", 16), bg="gray", fg="white",
                    width=15, height=2, command=self.edit_order)    
        
        self.button_close = tk.Button(buttons_frame, text="Close Order", font=("Arial", 16), bg="gray", fg="white",
                    width=15, height=2, command=self.close_order)
            
        if self.userType == "Admin":
            tk.Button(buttons_frame, text="Delete Order", font=("Arial", 16), bg="gray", fg="white",
                      width=15, height=2, command=self.delete_order).grid(row=6, column=1, pady=10)
        ...
        self.orders_list.heading("Created At", text="Last Edit")
        self.orders_list.heading("Comment", text="Comment")
        self.orders_list.heading("Status", text="Status")
        self.orders_list.bind("<<TreeviewSelect>>", self.on_select)
    ...
    def on_select(self,event):
        ...
        if selected_item:
            ...
            if order_data[3] == "Otwarte":
                self.button_close.grid(row=4, column=1, pady=10)
                self.button_edit.grid(row=3, column=1, pady=10)
        else:
